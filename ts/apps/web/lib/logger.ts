 ...
 * API logger wired in `lib/planetary.ts` guards on `typeof window` before
...
interface InterceptableClient {
  interceptors: {
    ...
    // without `lib/logger.ts` having to import from `@planetary/api-client`.
    request: {
      use(fn: (request: unknown, options: unknown) => unknown): void
    }
    response: {
      use(
        fn: (response: unknown, request: unknown, options: unknown) => unknown
      ): void
    }
    error: {
      use(
 ...
 * have to import from `@planetary/api-client` (only `lib/planetary.ts` may).
...
export function attachApiLogger(
  ...
  c.interceptors.response.use((response, request) => {
    ...
    log.info(
      { status: res.status, method, url, duration },
  ...
  c.interceptors.error.use((error, response, request) => {
    ...
    const level: "warn" | "error" =
      status === 0 || status >= 500 ? "error" : "warn"
    log[level](
